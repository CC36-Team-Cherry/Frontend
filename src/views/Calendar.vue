  ...
  <div class="flex h-screen bg-gray-100">
    ...
    <div class="bg-white shadow-sm border border-slate-200 rounded-lg p-3 m-2 flex flex-col w-72">
      ...
      <div class="flex mb-3">
        ...
        <button @click="tab = 'pto', attendanceType = 'pto'"
          :class="{ 'bg-blue-500 text-white': tab === 'pto', 'text-blue-500': tab !== 'pto' }"
          class="px-4 py-2 text-medium font-bold flex-1 rounded">{{ $t('calendar.types.pto') }}</button>
      </div>
      ...
      <div v-if="tab === 'attendance'">
        <div class="mb-3">
          ...
          <input type="text" placeholder="YYYY-MM-DD - YYYY-MM-DD" v-model="selectionRange"
        ...
        <div class="mb-3">
          ...
          <input type="time" v-model="startTime" class="border border-gray-300 rounded p-1 text-sm w-full" />
        </div>
        <div class="mb-3">
          <label class="block mb-1 font-bold text-medium">{{ $t('calendar.endTime') }}</label>
          <input type="time" v-model="endTime" class="border border-gray-300 rounded p-1 text-sm w-full" />
        </div>
        <div v-if="attendanceType === 'general'" class="mb-3">
          ...
          <div class="flex gap-2 items-center">
            ...
            <div class="flex items-center">
              <button @click="decreaseBreakHours"
                class="px-2 py-1 rounded border text-sm bg-gray-100 border-gray-300 hover:bg-blue-500 hover:text-white">
                -
              </button>
              <input type="number" v-model="breakHours" @input="validateHours" @focus="selectAll($event)"
                class="no-spinner px-3 py-1 text-center border text-sm bg-white border-gray-300 w-10" min="0" max="23">
              <button @click="increaseBreakHours"
                class="px-2 py-1 rounded border text-sm bg-gray-100 border-gray-300 hover:bg-blue-500 hover:text-white">
                +
              </button>
            ...
            <!-- Minutes Input -->
            <div class="flex items-center">
              <button @click="decreaseBreakMinutes"
                class="px-2 py-1 rounded border text-sm bg-gray-100 border-gray-300 hover:bg-blue-500 hover:text-white">
                -
              </button>
              <input type="number" v-model="breakMinutes" @input="validateMinutes" @focus="selectAll($event)"
                class="no-spinner px-3 py-1 text-center border text-sm bg-white border-gray-300 w-10" min="0" max="59">
              <button @click="increaseBreakMinutes"
                class="px-2 py-1 rounded border text-sm bg-gray-100 border-gray-300 hover:bg-blue-500 hover:text-white">
                +
              </button>
        ...
        <button @click="logAttendance" :class="{
          'bg-blue-500 text-white py-1 px-3 rounded hover:bg-blue-600 w-full text-base mb-3': isFormValid,
          'bg-gray-300 text-gray-500 py-1 px-3 rounded w-full cursor-not-allowed text-base mb-3': !isFormValid
        }">
          {{ $t('calendar.logAttendance') }}
        </button>
        <button @click="deleteGeneralAttendance" :class="{
          'bg-gray-500 text-gray-300 py-1 px-3 rounded w-full text-base mb-3 cursor-not-allowed': !selectionRange,
      ...
      <div v-if="tab === 'pto'">
        ...
        <div class="mb-3">
          ...
          <select v-model="attendanceType" class="border border-gray-300 rounded p-1 text-sm w-full">
            ...
            <optgroup v-if="specialPtos.length > 0" label="Special PTO">
              <option v-for="specialPto in specialPtos" :key="specialPto.id" :value="specialPto.attendanceType">
                {{ specialPto.type }}
              </option>
            </optgroup>
          </select>
        ...
        <div class="mb-3">
          ...
          <select v-model="selectedSupervisorId" class="border border-gray-300 rounded p-1 text-sm w-full">
        ...
        <div class="mb-3">
          ...
          <input v-model="memo" type="text" :placeholder="$t('approval.enterMemo')"
            class="border border-gray-300 rounded p-1 text-sm w-full" />
        </div>
        <button @click="submitHandler" :class="{
          'bg-cyan-500 text-white py-1 rounded hover:bg-green-600 w-full text-base mb-3': isSubmitFormValid,
      ...
      <div class="flex flex-col items-center space-y-3 mt-auto">
        ...
        <button class="bg-green-500 text-white py-1 px-3 rounded hover:bg-gray-600 w-full text-xl mb-2 h-16"
          @click.stop="openSubmitMonthModal">
          {{ $t('calendar.monthlySubmit') }}
        </button>
      </div>

      <SubmitMonthModal :isVisible="isSubmitMonthModalVisible" :supervisors="supervisors"
        :selectedSupervisorId="selectedSupervisorId" :memo="memo" @submit="submitMonthApproval"
...
import { ref } from 'vue';
import SubmitMonthModal from '@/components/modal/SubmitMonthModal.vue';
